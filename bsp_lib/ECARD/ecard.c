...
{
unsigned char Block=36;

  switch(Idx)
  {
    case 0: Block=25; break;
    case 1: Block=26; break;
    case 2: Block=28; break;
    case 3: Block=29; break;
    case 4: Block=30; break;
    case 5: Block=32; break;
    case 6: Block=33; break;
    case 7: Block=34; break;
    case 8: Block=36; break;
    case 9: Block=37; break;
    case 10: Block=38; break;
    case 11: Block=40; break;
    case 12: Block=41; break;
    case 13: Block=42; break;
  }
  return(Block);
}
//==========================================================================
...
{
unsigned char Block=44;

    switch(Idx)
    {
      case 0: Block=44; break;
      case 1: Block=45; break;
      case 2: Block=46; break;
      case 3: Block=48; break;
      case 4: Block=49; break;
      case 5: Block=50; break;
      case 6: Block=52; break;
      case 7: Block=53; break;
      case 8: Block=54; break;
    }
    return(Block);
}
...
{
unsigned char BCC, i;
unsigned char buf[32],snr[10];
unsigned char IsOldFormat;
signed long int Value;
signed long int TempEtebar=0;
signed long int Payment;
...
if(ISO14443_SingleTagSelect(snr)) {if(ISO14443_SingleTagSelect(snr))  return(17);}

  if(ISO14443_LoginE2(0,0x10)) 
    if(ISO14443_LoginE2(0,0x10))
      return(17);

  if(ISO14443_ReadBlock(1, buf)) return(18); 
  ...
  for(i=0; i<4; i++) if(buf[i+4]!=(buf[i]^0xFF)) IsOldFormat=0;  
  if((buf[12]!=0)||(buf[13]!=0xFF)||(buf[14]!=0)||(buf[15]!=0xFF)) IsOldFormat=0;
  if((CurECardInfo.UC==6374)&&(CurECardInfo.CardID>=30001)&&(CurECardInfo.CardID<=40000))
    IsOldFormat=0;
  if(IsOldFormat)
  {                                      
    CurECardInfo.Etebar=buf[3]; CurECardInfo.Etebar<<=8; 
  ...
  CurECardInfo.DeviceID=CurECardInfo.EtebarBlock[14]; CurECardInfo.DeviceID<<=8;    
...
unsigned char buf[32];
  ...
  if(ISO14443_LoginE2(CreditSector,CreditLogin)) return(3);
  if(ISO14443_WriteBlock(CreditBlock, CurECardInfo.EtebarBlock)) return(4);
  if(ISO14443_ReadBlock(CreditBlock, buf)) return(5);
...
//==========================================================================
unsigned char ExecTransaction(unsigned char    Type,      unsigned long int *CardID,     unsigned int  *UC,      unsigned int       Payment,
                              signed long int  MinEtebar, unsigned long int  MaxEtebar,  unsigned int  DeviceID, signed long int   *Etebar,    
															signed long int *PreEtebar, unsigned short    *LastDevice, unsigned char *LastOP,  unsigned char     *OPIndex) {
...
unsigned int _ID,_UC;
	...
	if(ISO14443_SingleTagSelect(snr)) {if(ISO14443_SingleTagSelect(snr))  return(1);}
	
  if(ISO14443_LoginE2(0,0x10)) return(1);
  if(ISO14443_ReadBlock(1, buf)) return(2);  
...
{
unsigned int SelectTimeout=0;
unsigned char Res, j, i, ShowMsg, CardIsSuspend=0,_CoderType=ctNone;
signed int Diff;
unsigned char snr[10];
unsigned char buf[32];
unsigned int Grouh;
...
signed long int TempEtebar=0;
  ...
  *PreEtebar=0;
  ...
  {      
    ...
    if(!FirstLoop)
    {            
      if(ISO14443_SingleTagSelect(snr))  {if(ISO14443_SingleTagSelect(snr)) { SelectTimeout++; continue; }}
  ...
  *PreEtebar=CurECardInfo.PreEtebar;  
    ...
    *OPIndex=CurECardInfo.OPIndex;
    ...
    {  
      for(i=0; i<4; i++)
        if(CurECardInfo.CardSerialNumber[i]!=Old_SuspendECardsInfo[j].CardSerialNumber[i])
          break;  
      
      if(i>=4)
      {   
     
        Diff=(((Hour*60)+Min)-((Old_SuspendECardsInfo[j].Hour*60)+Old_SuspendECardsInfo[j].Min));
  ...
  { 
    *Etebar=CurECardInfo.Etebar;  
    ...
    CurECardInfo.Min=Min;
    CurECardInfo.DeviceID=DeviceID;
    if(CurECardInfo.OPIndex>13) CurECardInfo.OPIndex=0; 
  ...
  {  
    ...
    {         
      if(ISO14443_SingleTagSelect(snr)) {if(ISO14443_SingleTagSelect(snr)){ SelectTimeout++; continue; }}
      if((snr[0]==0)&&(snr[1]==0)&&(snr[2]==0)&&(snr[3]==0)) { SelectTimeout++; continue; }
