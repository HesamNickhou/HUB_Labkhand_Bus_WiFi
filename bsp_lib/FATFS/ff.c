...
//        printf("\n\rfind_volume   vol=%d,path=%s,Fsid=%d",vol,path,Fsid);
...
// printf("\n\r3 fmt=%d,fs->csize=%d,(tsect - sysect)=%d",fmt,fs->csize,(tsect - sysect));
 ...
 //printf("\n\rfs->id=%d,fs->fs_type=%d ",fs->id,fs->fs_type,fs->last_clust , fs->free_clust);        
...
)
{
	...
	res = find_volume(&dj.fs, &path, (BYTE)(mode & ~FA_READ));
        //printf("\n\r* f_open res=%d ,path=%s",res,path);
#else
	mode &= FA_READ;
