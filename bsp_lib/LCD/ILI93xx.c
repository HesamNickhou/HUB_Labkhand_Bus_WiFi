...
delay_ms(50); // delay 50 ms 
    ...
    LCD_WriteReg(0xe0,0x01);    //START PLL
    LCD_WriteReg(0xe0,0x03);    //LOCK PLL
    
    LCD_WR_REG(0xb0);        //LCD specification
    LCD_WR_DATA(0x20);            //SET TFT MODE & hsync+Vsync+DEN MODE
    LCD_WR_DATA(0x00);            //SET TFT MODE & hsync+Vsync+DEN MODE
		//DISP_HOR_RESOLUTION
		
    LCD_WR_DATA(0x03);            //SET horizontal size=800-1 HightByte
    LCD_WR_DATA(0x1f);            //SET horizontal size=800-1 LowByte
    LCD_WR_DATA(0x01);            //SET vertical size=480-1 HightByte
    LCD_WR_DATA(0xdf);            //SET vertical size=480-1 LowByte
    LCD_WR_DATA(0x00);            //SET even/odd line RGB seq.=RGB
    delay_ms(5);
    
    LCD_WriteReg(0xf0,0x00);    //SET pixel data I/F format=8bit
    
    //Command_Write(0x3a,0x60);   // SET R G B format = 6 6 6
    
    LCD_WR_REG(0xe6);           //PLL setting for PCLK, depends on resolution
    LCD_WR_DATA(0x00);
    LCD_WR_DATA(0xff);
    LCD_WR_DATA(0xbe);
    
    LCD_WR_REG(0xb4);        //SET HBP,
    LCD_WR_DATA(0x03);            //SET HSYNC Total=928 HightByte
    LCD_WR_DATA(0xa0);            //SET HSYNC Total=928 LowByte
    LCD_WR_DATA(0x00);            //SET HBP 46 HightByte
    LCD_WR_DATA(0x2e);            //SET HBP 46 LowByte
    LCD_WR_DATA(0x30);            //SET VBP 48 
    LCD_WR_DATA(0x00);            //SET Hsync pulse start position  (15 HighByte)
    LCD_WR_DATA(0x0f);            //15 LowByte
    LCD_WR_DATA(0x00);            //SET Hsync pulse subpixel start position
    
    LCD_WR_REG(0xb6);         //SET VBP,
    LCD_WR_DATA(0x02);            //SET Vsync total=525 HighByte
    LCD_WR_DATA(0x0d);            // 525 LowByte
    LCD_WR_DATA(0x00);            //SET VBP=16 HighByte
    LCD_WR_DATA(0x10);            // 16 LowByte
    LCD_WR_DATA(0x10);            //SET Vsync pulse 16
    LCD_WR_DATA(0x00);            //SET Vsync pulse start position (8 HighByte)
    LCD_WR_DATA(0x08);            //8 LowByte
    ...
    LCD_WriteReg(0x36,0x00);    //Rotation
    ...
    //LCD_WR_REG(SSD1963_CMD_SET_PIXEL_FORMAT);
    //LCD_WR_DATA(0x50); // set 16bpp

    // Rotate screen and rgb
    //LCD_WR_REG(SSD1963_CMD_SET_ADDRESS_MODE);
    //LCD_WR_DATA(0x00); // top to bottom, left to right, normal,  top to bottom, rgb, left to right, normal

    // Set pixel data interface
    //LCD_WR_REG(SSD1963_CMD_SET_PIXEL_DATA_INTERFACE);
    //LCD_WR_DATA(0x03); // 16-bit(565 format)
    //LCD_WR_DATA(0x00); // 8-bit data for 16bpp

    // Exit Idle mode
    //LCD_WR_REG(SSD1963_CMD_EXIT_IDLE_MODE);

    //LCD_WR_REG(SSD1963_CMD_SET_DISPLAY_ON);   
      


//ssd1963_set_area(0, 0, 799, 479);
...
void LCD_Clear(u16 color)
{
	...
	unsigned int lineWidth = 800 + 1,height = 480 + 1, w;
	

	// Set the start column and page addresses
	SetLCDStartPixelLineAddress(0,0,800,480);
    while(lineWidth--)
	{
	    for(w  =  0;w < height;w++)
		{    
