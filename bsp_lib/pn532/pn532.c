...
unsigned char _inListedTag=0;
unsigned char _usingSPI=1;
...
void begin() {
    ...
    sendCommandCheckAck(pn532_packetbuffer, 1, 2000);
...
*/
/**************************************************************************/
void PrintHex(const unsigned char * data, const uint32_t numBytes)
{
  uint32_t szPos;
...
*/
/**************************************************************************/
void PrintHexChar(const unsigned char * data, const uint32_t numBytes)
{
  uint32_t szPos;
  for (szPos=0; szPos < numBytes; szPos++)
  {
...
int offset, i;
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 1, 2000)) {
    return 0;
  }
...
int offset;
  ...
  #ifdef PN532DEBUG
    printf("\n\rWriting P3 GPIO: "); PN532DEBUGPRINT.println(pn532_packetbuffer[1], HEX);
  #endif

  // Send the WRITEGPIO command (0x0E)
  if (! sendCommandCheckAck(pn532_packetbuffer, 3, 2000))
    return 0x0;
...
int p3offset;
  ...
  p3offset = _usingSPI ? 6 : 7;

  #ifdef PN532DEBUG
    printf("\n\rReceived: ");
    PrintHex(pn532_packetbuffer, 11);
    PN532DEBUGPRINT.println();
    printf("\n\rP3 GPIO: 0x"); PN532DEBUGPRINT.println(pn532_packetbuffer[p3offset],   HEX);
...
int offset;
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 4, 2000))
    return FALSE;
...
bool setPassiveActivationRetries(uint8_t maxRetries) {
  ...
  #ifdef MIFAREDEBUG
    printf("\n\rSetting MxRtyPassiveActivation to "); PN532DEBUGPRINT.print(maxRetries, DEC); printf("\n\r ");
  #endif

  if (! sendCommandCheckAck(pn532_packetbuffer, 5, 2000))
    return 0x0;  // no ACK
...
uint8_t i;
  ...
  if (!_usingSPI) {
    #ifdef PN532DEBUG
      printf("\n\rWaiting for IRQ (indicates card presence)");
    #endif
    if (!waitready(timeout)) {
      #ifdef PN532DEBUG
        printf("\n\rIRQ Timeout");
  ...
  #ifdef MIFAREDEBUG
    printf("\n\rFound "); PN532DEBUGPRINT.print(pn532_packetbuffer[7], DEC); printf("\n\r tags");
  #endif
  ...
  #ifdef MIFAREDEBUG
    printf("\n\rATQA: 0x");  PN532DEBUGPRINT.println(sens_res, HEX);
    printf("\n\rSAK: 0x");  PN532DEBUGPRINT.println(pn532_packetbuffer[11], HEX);
  #endif
  ...
  *uidLength = pn532_packetbuffer[12];
  ...
  for (i=0; i < pn532_packetbuffer[12]; i++)
  {
    ...
    #ifdef MIFAREDEBUG
      printf("\n\r 0x");PN532DEBUGPRINT.print(uid[i], HEX);
    #endif
  }
  #ifdef MIFAREDEBUG
    PN532DEBUGPRINT.println();
...
uint8_t i;
  ...
  if (sendLength > PN532_PACKBUFFSIZ-2) {
    #ifdef PN532DEBUG
      printf("\n\rAPDU length too long for packet buffer");
    #endif
    return FALSE;
  }
  ...
  if (!waitready(1000)) {
    #ifdef PN532DEBUG
      printf("\n\rResponse never received for APDU...");
    #endif
    return FALSE;
  }
  ...
  if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff) {
    ...
    if (pn532_packetbuffer[5]==PN532_PN532TOHOST && pn532_packetbuffer[6]==PN532_RESPONSE_INDATAEXCHANGE) {
      ...
      *responseLength = length;
      ...
      printf("\n\rDon't know how to handle this command: ");
...
bool inListPassiveTarget() {
  ...
  #ifdef PN532DEBUG
    printf("\n\rAbout to inList passive target");
  #endif

  if (!sendCommandCheckAck(pn532_packetbuffer,3,1000)) {
    #ifdef PN532DEBUG
      printf("\n\rCould not send inlist message");
    #endif
    return FALSE;
  }
  ...
  if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff) {
    ...
    if (pn532_packetbuffer[5]==PN532_PN532TOHOST && pn532_packetbuffer[6]==PN532_RESPONSE_INLISTPASSIVETARGET) {
      ...
      printf("\n\rTag number: ");
      //.println(_inListedTag);

      return TRUE;
    } else {
      #ifdef PN532DEBUG
        printf("\n\rUnexpected response to inlist passive host");
      #endif
      return FALSE;
    }
  }
...
uint8_t mifareclassic_AuthenticateBlock (uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
{
  ...
  #ifdef MIFAREDEBUG
    printf("\n\rTrying to authenticate card ");
    PrintHex(_uid, _uidLen);
    printf("\n\rUsing authentication KEY ");PN532DEBUGPRINT.print(keyNumber ? 'B' : 'A');printf("\n\r: ");
    PrintHex(_key, 6);
  #endif
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 10+_uidLen, 2000))
    return 0;
...
{
  #ifdef MIFAREDEBUG
    printf("\n\rTrying to read 16 bytes from block ");PN532DEBUGPRINT.println(blockNumber);
  #endif
  ...
  {
    #ifdef MIFAREDEBUG
      printf("\n\rFailed to receive ACK for read command");
    #endif
    return 0;
  }
...
{
  #ifdef MIFAREDEBUG
    printf("\n\rTrying to write 16 bytes to block ");PN532DEBUGPRINT.println(blockNumber);
  #endif
  ...
  /* Send the command */
  if (! sendCommandCheckAck(pn532_packetbuffer, 20, 2000))
  {
    #ifdef MIFAREDEBUG
      printf("\n\rFailed to receive ACK for write command");
    #endif
    return 0;
  }
  GUI_Delay(10);
...
{
uint8_t len;	
uint8_t sectorbuffer1[16] = {0x00, 0x00, 0x03, 0, 0xD1, 0x01, 0, 0x55, 0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
uint8_t mifareultralight_ReadPage (uint8_t page, uint8_t * buffer)
{
  ...
  {
    #ifdef MIFAREDEBUG
      printf("\n\rPage value out of range");
    #endif
    return 0;
  }

  #ifdef MIFAREDEBUG
    printf("\n\rReading page ");PN532DEBUGPRINT.println(page);
  #endif
  ...
  {
    #ifdef MIFAREDEBUG
      printf("\n\rUnexpected response reading block: ");
      PrintHexChar(pn532_packetbuffer, 26);
    #endif
    return 0;
  }
  ...
  /* Display data for debug if requested */
  #ifdef MIFAREDEBUG
    printf("\n\rPage ");PN532DEBUGPRINT.print(page);printf("\n\r:");
...
uint8_t mifareultralight_WritePage (uint8_t page, uint8_t * data)
{
  ...
  #ifdef MIFAREDEBUG
    printf("\n\rTrying to write 4 byte page");PN532DEBUGPRINT.println(page);
  #endif
  ...
  /* Send the command */
  if (! sendCommandCheckAck(pn532_packetbuffer, 8, 2000))
  {
    #ifdef MIFAREDEBUG
      printf("\n\rFailed to receive ACK for write command");
    #endif
...
uint8_t i;
    ...
    #endif
    R_CS=0;
    GUI_Delay(2);     // or whatever the delay is for waking up the board
    ...
    #ifdef PN532DEBUG
      printf("\n\r 0x"); PN532DEBUGPRINT.print(PN532_PREAMBLE, HEX);
      printf("\n\r 0x"); PN532DEBUGPRINT.print(PN532_PREAMBLE, HEX);
      printf("\n\r 0x"); PN532DEBUGPRINT.print(PN532_STARTCODE2, HEX);
      printf("\n\r 0x"); PN532DEBUGPRINT.print(cmdlen, HEX);
      printf("\n\r 0x"); PN532DEBUGPRINT.print(~cmdlen + 1, HEX);
      printf("\n\r 0x"); PN532DEBUGPRINT.print(PN532_HOSTTOPN532, HEX);
    #endif
    ...
    for (i=0; i<cmdlen-1; i++) {
      ...
      checksum += cmd[i];
      #ifdef PN532DEBUG
        printf("\n\r 0x"); PN532DEBUGPRINT.print(cmd[i], HEX);
      #endif
    }
    ...
    #ifdef PN532DEBUG
      printf("\n\r 0x"); PN532DEBUGPRINT.print(~checksum, HEX);
      printf("\n\r 0x"); PN532DEBUGPRINT.print(PN532_POSTAMBLE, HEX);
      PN532DEBUGPRINT.println();
    #endif
}
/************** low level SPI */
