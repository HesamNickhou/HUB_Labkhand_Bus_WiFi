...
phStatus_t phhalHw_Rc632_SetBitMask(phhalHw_Rc632_DataParams_t* pDataParams, uint8_t reg, uint8_t mask)
{
    ...
    phhalHw_Rc632_ReadRegister(reg, &tmp);
    phhalHw_Rc632_WriteRegister(reg, tmp | mask);  /* set bit mask */
...
phStatus_t phhalHw_Rc632_ClearBitMask(phhalHw_Rc632_DataParams_t* pDataParams, uint8_t reg, uint8_t mask)
{
    ...
    phhalHw_Rc632_ReadRegister(reg, &tmp);
    phhalHw_Rc632_WriteRegister(reg, tmp & (uint8_t)~(uint8_t)mask);  /* clear bit mask */
    return PH_ERR_SUCCESS;
}
                                     ...
                                     )
{
    ...
    phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_INT_REQ, &bRegister);

    /* Enable IRQ interrupt sources */
    phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_INT_EN, &bRegister);
    bRegister |= bIrqWaitFor;
    phhalHw_Rc632_WriteRegister(PHHAL_HW_RC632_REG_INT_EN, bRegister);
    ...
    /* start the command */
    phhalHw_Rc632_WriteRegister(PHHAL_HW_RC632_REG_COMMAND, bCmd);
    ...
    if ((bIrqWaitFor & PHHAL_HW_RC632_BIT_TIMERIRQ) && (bIrqReg & PHHAL_HW_RC632_BIT_TIMERIRQ))
    {
        ...
        phhalHw_Rc632_WriteRegister(PHHAL_HW_RC632_REG_TIMER_CONTROL, bRegister);
                                              ...
                                              uint8_t bBlockNo)
{
    ...
    phhalHw_Rc632_WriteRegister(PHHAL_HW_RC632_REG_CHANNEL_REDUND, 0x07); /* RxCRC disable, TxCRC, Parity enable */
    ...
    phhalHw_Rc632_WriteRegister(PHHAL_HW_RC632_REG_CHANNEL_REDUND, 0x03); /* RxCRC, TxCRC disable, Parity enable */
    bIrqWaitFor = PHHAL_HW_RC632_BIT_TXIRQ | PHHAL_HW_RC632_BIT_TIMERIRQ | PHHAL_HW_RC632_BIT_IDLEIRQ;
                                   ...
                                   )
{
    ...
    phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_FIFO_LENGTH, &bFifoLen);
    ...
    if (wLength > 0)
    {
        ...
        {
            phhalHw_Rc632_WriteRegister(PHHAL_HW_RC632_REG_FIFO_DATA, pData[wIndex]);
        }
    }
                                  ...
                                  )
{
    ...
    {
        phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_FIFO_DATA, &pData[bIndex]);
    }
                                 ...
                                 )
{
    ...
    phhalHw_Rc632_WriteRegister(PHHAL_HW_RC632_REG_INT_EN, 0x7F);          /* disable all interrupts */
    /* Enable IRQ interrupt sources */
    phhalHw_Rc632_WriteRegister(PHHAL_HW_RC632_REG_INT_EN, bIrqWaitFor | PHHAL_HW_RC632_BIT_IRQ);  /* enable neccessary interrupts */

    /* wait until an IRQ occurs */
    do
    {
        phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_PRIMARY_STATUS, &bRegister);
    }
    ...
    phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_INT_REQ, pIrqReg);
    return PH_ADD_COMPCODE(PH_ERR_SUCCESS, PH_COMP_HAL);
}
                                      ...
                                      )
{
    ...
    uint8_t            bTValue;
    float32_t        fTime;
    float32_t        fTimeCurrent;
    uint8_t            bTPrescaler;
    uint16_t        wDelay;

    /* Get T reload value */
    phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_TIMER_RELOAD, &bRegister);
    bTReload = bRegister;

    /* Get T prescaler value */
    phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_TIMER_CLOCK, &bRegister);
    bTPrescaler = bRegister;

    /* Get T counter value */
    if ((wExchangeStatus & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT)
    {
        phhalHw_Rc632_ReadRegister(PHHAL_HW_RC632_REG_TIMER_VALUE, &bRegister);
        bTValue = bRegister;
    }
    else
    {
