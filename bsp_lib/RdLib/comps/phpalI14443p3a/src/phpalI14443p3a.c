...
phStatus_t phpalI14443p3a_Init(
...
{
    return phpalI14443p3a_RequestAEx(pDataParams, PHPAL_I14443P3A_REQUEST_CMD, pAtqa);
}
...
{
    return phpalI14443p3a_RequestAEx(pDataParams, PHPAL_I14443P3A_WAKEUP_CMD, pAtqa);
}
    ...
    )
{
    ...
    phStatus_t   statusTmp;
    uint8_t      bIsSelect;
    uint8_t      bCmdBuffer[7];
    uint8_t *    pRcvBuffer;
    uint16_t     wSndBytes;
    uint16_t     wRcvBytes;
    uint16_t     wRcvBits;
    uint8_t      bUidStartIndex;
    uint8_t      bBitCount;
...
{
    uint8_t  bDummy;
    return phpalI14443p3a_Anticollision(pDataParams, bCascadeLevel, pUidIn, 0x40, pSak, &bDummy);
}
    ...
    )
{
    ...
    *pMoreCardsAvailable = PH_OFF;
    ...
    for (bCascadeIndex = 0; bCascadeIndex < 3; bCascadeIndex++)
    {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phpalI14443p3a_Select(pDataParams, bCascadeLevel, bUid, pSak));
...
phStatus_t phpalI14443p3a_RequestAEx(
                                        phpalI14443p3a_DataParams_t * pDataParams,
                                        uint8_t bReqCode,
                                        uint8_t * pAtqa
                                        )
