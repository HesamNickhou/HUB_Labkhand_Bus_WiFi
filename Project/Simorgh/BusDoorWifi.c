...
void ShowCardingResult(char Mode, 
											 unsigned int Pay,
											 unsigned int _UC,
											 unsigned int _ID,
 ...
 //.....................................................................................
	if (Mode == 0) {
		...
		sprintf(str, "%d", Pay);
		PutText(0, 70, 
			272 + SCREENRESIZE - (LCDType == LCD480_272 ? SCREENRESIZE : 0), 
			400, str, GUI_TA_CENTER);
		...
		PutText(0, 20 + 15, 
			272 + SCREENRESIZE - (LCDType == LCD480_272 ? SCREENRESIZE : 0), 
			20 + 15 + 50, "اعتبار کافي نمي باشد ", GUI_TA_CENTER);

		GUI_SetColor(GUI_BLACK);	
		PutText(0, 225 - 25, 
			272 + SCREENRESIZE - (LCDType == LCD480_272 ? SCREENRESIZE : 0), 
			225 + 50, "تومان", GUI_TA_CENTER);
...
unsigned char SetDateAndTime(unsigned char Y, unsigned char M, unsigned char D, 
...
unsigned short ShowErrorKashan(unsigned int Status) {
  switch (Status)
  {
    ...
    case 0x9920: 
    case 0x9930: Status = S_OutOfRange; break;
...
char ProcessCard(unsigned char MifareType, unsigned char *snr) {
  ...
  if (MifareType == 2) {
    ...
    else if ((Status != 0) && (Status != 0x9714))
			Status = ShowErrorKashan(Status);
		else {
        #ifdef CLRC_CHIP
  ...
  switch (Status) {
    ...
    case S_Success: {
      ...
      break;
		...
		case S_OutOfRange: {
      ShowCardingResult(1, Etebar, Config.UC, ID, Status, "");
      InfoCounter = 2;
      break;
		}
		...
		/*
		 {
				...
				"deviceId": 		 "integer", // from tr.deviceId
				"deviceType": 	 "integer", // from tr.deviceType.ordinal
				"uc": 					 "integer", // from tr.uc
				"operatorId": 	 "integer", // from tr.operatorId ?: 0
				"lineId": 			 "integer", // from tr.lineId
				"cardId": 			 "integer", // from tr.cardId
				"preCardCredit": "long", 		// from tr.preCardCredit
				"price": 				 "long", 		// from tr.price
				"cardCredit": 	 "long", 		// from tr.cardCredit
				"date": 				 "long", 		// from tr.date (timestamp)
...
unsigned char GetDateRange(unsigned short *SYear, unsigned char *SMonth, 
	unsigned char *SDay, unsigned short *EYear, unsigned char *EMonth, 
	unsigned char *EDay) {
...
void ProcessKeysAlef(unsigned int Key) {
  switch (Key) {
    ...
    case BA2: break;
    case BA3: break;
		case BA4: /*frmTransactionsList();*/ break;
    case BA5:  break;
    case BA6:  break;
    case BA7:  break;
    case BA8:  break;
