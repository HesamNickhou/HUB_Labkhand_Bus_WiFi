...
unsigned int FirmwareReqTimer=10;
...
unsigned int BlackList_LastIdx=0;
unsigned int BlackList_Index=0;
...
unsigned char MapView = 0;
unsigned short OldGPSSpeed;
...
unsigned char NumGPSSent=0,cntDriverInfo=30; 
unsigned int WaitForAddress=0;
unsigned int GWaitForAddress=0;
...
static const GUI_POINT _aPolig3[] = {
 ...
 PutText(0, 50, 272, 350, str, GUI_TA_CENTER);
  ...
  sprintf(str,"ک :%d",Indicators.OffTransactions);
...
void ProcessCommands(void) {
  ...
  switch (GlobalBuffer[6]) //Command #
  {
        ...
        case 80: // Receive Device Configuration
          Idx=7;
          /*Config.DeviceID=GlobalBuffer[Idx]+(GlobalBuffer[Idx+1]*256);*/ Idx += 2;   
          /*Config.Grouh=GlobalBuffer[Idx]+(GlobalBuffer[Idx+1]*256);*/ Idx += 2;   
          //Config.UC=DeviceUC;/*GlobalBuffer[Idx]+(GlobalBuffer[Idx+1]<<8);*/ Idx += 2;   
...
unsigned char ProcessPacket(unsigned char Port) {
  ...
  for (i = 0; i < BufferLen; i++)
    if ((GlobalBuffer[i] == STX) &&
       ((GlobalBuffer[i+1] == 1) || (GlobalBuffer[i+1] == 4) || (GlobalBuffer[i+1] == 10) || (GlobalBuffer[i+1] == 13) || (GlobalBuffer[i+1] == 15)))
      break;
  //..................................................
	/*	
  Err=0;
  ...
  */		
  ...
  ConnectionError = 0;
	/*
	GUI_DrawGradientRoundedV(18, 6, 200, 28,6,  GUI_BLACK,GUI_BLACK);		 
	LoadFont(FontBKoodak40);
		
  GUI_SetColor(GUI_YELLOW); 		 
	sprintf(str,"%d,%d,%d,%d,%d,%d,%d",GlobalBuffer[0],GlobalBuffer[1],GlobalBuffer[2],GlobalBuffer[3],GlobalBuffer[4],GlobalBuffer[5],GlobalBuffer[6]);
  PutText(20, 0, 200, 28, str, GUI_TA_LEFT); 
...
*/
  ...
  switch (GlobalBuffer[1]) {
    ...
    case 3:  //Alive Ack
      ...
      Address += GlobalBuffer[8]; Address <<= 8;
      Address += GlobalBuffer[7]; Address <<= 8;
      Address += GlobalBuffer[6];
      ...
      if (Address != Indicators.OperatorID) {
        Indicators.OperatorID = Address;    
			...
			if ((Year != (GlobalBuffer[10] + 1300)) || (Month != GlobalBuffer[11]) || (Day != GlobalBuffer[12]) || (Hour != GlobalBuffer[13]) || (Min != GlobalBuffer[14])) {
        SetDateAndTime(GlobalBuffer[10],GlobalBuffer[11],GlobalBuffer[12],GlobalBuffer[13],GlobalBuffer[14],GlobalBuffer[15],0);
        Refresh=1;
      } 
    ...
    case 4:  //Device Alive
      ...
      if (i >= 52) {  
        ...
        Address=GlobalBuffer[11]; Address<<=8;  
        ...
        {  
          ...
          j=GlobalBuffer[Idx+3]; j<<=8;  
          j += GlobalBuffer[Idx+2]; j<<=8;  
          j += GlobalBuffer[Idx+1]; j<<=8;  
          j += GlobalBuffer[Idx];
					Idx += 4;
      ...
      switch (GlobalBuffer[6])
      {
       ...
       //printf("\n\r Data Sent Add: %d [%d]",WaitForAddress, Sector);
            ...
            if (LCDType==LCD480_272)
              ...
              GUI_DrawGradientV(130+SCREENRESIZE, 4, 220+SCREENRESIZE, 28,0x000F0000,0x00070000);							
              GUI_SetColor(GUI_WHITE); 
              sprintf(str,"ک : %d",Indicators.OffTransactions);
              PutText(130+SCREENRESIZE, 0, 220+SCREENRESIZE, 25, str, GUI_TA_RIGHT);							
			      }
...
char ProcessCard(unsigned char MifareType,unsigned char *snr) {
	...
	signed long int Etebar, PreEtebar,RemEtebar,MinCharge=0,TempInt;
	char str[100],Try=0;
	static unsigned char RepeatedCard_Counter=0;
	unsigned char _CRC, Loop, SerialChip, Key, MemRes=0;
	unsigned int j;    
	unsigned char TicketBuffer[100],TicketBuffer2[32],EmptyCard=0;
...
void ProcessPeriodicTasks(void) {
  ...
  if (Min != OldMin) {
   RTC_Get();
	 //ShowPageAlef();
	 GUI_SetColor(0);
	 GUI_FillRoundedRect(8, 348+(LCDType==LCD480_272?55:0), 272-8+SCREENRESIZE-(LCDType==LCD480_272?SCREENRESIZE:0), 430+(LCDType==LCD480_272?50:0),6);
  ...
  if (++DeviceInfoSendInterval > (5 * 60)) {
  ...
  if (DataCurrentlySend==0) {
    if (Connected2BCU) {
      if (++Connected2BCU > 8) {
				Connected2BCU = 0;
				ConnectionMark(GUI_RED); 
			}
...
void InitializeAlef(void) {
  ...
  Config.DeviceInfoSendInterval=30; ///////////////////////////90
	LoadRecordsInfo();
  Config.LNG=0;
  GPSValidData=0;  
  ...
  //Save Startup time
  //x=GPS_CheckSystemTime2(0); //Check GPS time (my be gps connected to backup battery)
